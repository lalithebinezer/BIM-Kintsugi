...
// Find the start of "Structured Docking Overrides" and remove it entirely
const dockIndex = css.indexOf('/* Structured Docking Overrides */');
if (dockIndex !== -1) {
    css = css.substring(0, dockIndex);
}

// Remove HUD spacing overrides
const hudIndex = css.indexOf('/* HUD Spacing for Docked Layout */');
if (hudIndex !== -1) {
    css = css.substring(0, hudIndex);
}
